...
public class CmppSubmitRequestMessageCodec implements ICodec {
    ...
    public IMessage decode(IHeader header, ByteBuf bodyBuffer) {
        ...
        message.setFeeTerminalId(bodyBuffer.readCharSequence(21, CmppConstants.DEFAULT_TRANSPORT_CHARSET).toString().trim());
        ...
        for (int i = 0; i < destUsrTl; i++) {
            destTermId[i] = bodyBuffer.readCharSequence(21, CmppConstants.DEFAULT_TRANSPORT_CHARSET).toString().trim();
        }
        ...
        message.setReserve(bodyBuffer.readCharSequence(8, CmppConstants.DEFAULT_TRANSPORT_CHARSET).toString().trim());
    ...
    public ByteBuf encode(IMessage msg, ByteBuf bodyBuffer) {
        ...
        bodyBuffer.writeBytes(CommonUtil.ensureLength(message.getFeeType().
                getBytes(CmppConstants.DEFAULT_TRANSPORT_CHARSET), 2));
