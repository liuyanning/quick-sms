...
import com.drondea.sms.message.smpp34.SmppDataSmResponseMessage;
 ...
 * @author: gengjinbiao
 **/
public class SmppDataSmResponseMessageCodec implements ICodec {

    private static final Logger logger = LoggerFactory.getLogger(SmppDataSmResponseMessageCodec.class);

    @Override
    public IMessage decode(IHeader header, ByteBuf bodyBuffer) throws Exception {
        SmppDataSmResponseMessage message = new SmppDataSmResponseMessage((SmppHeader) header);
        try {
    ...
    public ByteBuf encode(IMessage msg, ByteBuf bodyBuffer) throws Exception {
        SmppDataSmResponseMessage message = (SmppDataSmResponseMessage) msg;
        try {
