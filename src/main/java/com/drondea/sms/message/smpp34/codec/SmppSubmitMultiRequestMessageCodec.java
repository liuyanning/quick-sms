...
public class SmppSubmitMultiRequestMessageCodec implements ICodec {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SmppSubmitMultiRequestMessageCodec.class);

    @Override
    public IMessage decode(IHeader header, ByteBuf bodyBuffer) throws Exception {
        SmppSubmitMultiRequestMessage message = new SmppSubmitMultiRequestMessage((SmppHeader) header);
        try {
            ...
            message.setNumberOfDests(bodyBuffer.readByte());
            message.setDestFlag(bodyBuffer.readByte());
            ...
            message.setDlDestFlag(bodyBuffer.readByte());
            message.setDlName(SmppUtil.readNullTerminatedString(bodyBuffer));
    ...
    public ByteBuf encode(IMessage msg, ByteBuf bodyBuffer) {
        SmppSubmitMultiRequestMessage message = (SmppSubmitMultiRequestMessage) msg;
        try {
            ...
            bodyBuffer.writeByte(message.getNumberOfDests());
            bodyBuffer.writeByte(message.getDestFlag());
            ...
            bodyBuffer.writeByte(message.getDlDestFlag());
            SmppUtil.writeNullTerminatedString(bodyBuffer, message.getDlName());
