...
import com.drondea.sms.message.smpp34.SmppQueryBroadcastSmResponseMessage;
 ...
 * @author: gengjinbiao
 **/
public class SmppQueryBroadcastSmResponseMessageCodec implements ICodec {

    private static final Logger logger = LoggerFactory.getLogger(SmppQueryBroadcastSmResponseMessageCodec.class);

    @Override
    public IMessage decode(IHeader header, ByteBuf bodyBuffer) throws Exception {
        SmppQueryBroadcastSmResponseMessage message = new SmppQueryBroadcastSmResponseMessage((SmppHeader) header);
        try {
    ...
    public ByteBuf encode(IMessage msg, ByteBuf bodyBuffer) {
        SmppQueryBroadcastSmResponseMessage message = (SmppQueryBroadcastSmResponseMessage) msg;
        try {
