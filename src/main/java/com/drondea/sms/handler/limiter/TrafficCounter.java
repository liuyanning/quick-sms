 ...
 * {@code checkInterval}, and calls the {@link AbstractCounterLimitHandler#doAccounting(TrafficCounter)} method back.
...
public class TrafficCounter {
    ...
    final AtomicLong checkInterval = new AtomicLong(
            AbstractCounterLimitHandler.DEFAULT_CHECK_INTERVAL);
     ...
     */
    final AbstractCounterLimitHandler trafficShapingHandler;
    ...
    /**
     * Constructor with the {@link AbstractCounterLimitHandler} that hosts it, the {@link ScheduledExecutorService}
    ...
    /**
     * Constructor with the {@link AbstractCounterLimitHandler} that hosts it, the Timer to use, its
    ...
    public TrafficCounter(
            AbstractCounterLimitHandler trafficShapingHandler, ScheduledExecutorService executor,
    ...
    public long readTimeToWait(final long size, final long limitTraffic, final long maxTime, final long now) {
        ...
        //lastReadingTime上个周期的最后一个读的时间
        long pastDelay = Math.max(lastReadingTime - lastTimeCheck, 0);
        if (interval > AbstractCounterLimitHandler.MINIMAL_WAIT) {
            // Enough interval time to compute shaping，加入这条消息后需要等待的时间
            long time = sum * 1000 / limitTraffic - interval + pastDelay;
            if (time > AbstractCounterLimitHandler.MINIMAL_WAIT) {
